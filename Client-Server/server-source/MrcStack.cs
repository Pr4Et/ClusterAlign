...
namespace ClusterAlign
{
    public class MrcStack
    {
        ...
        public unsafe void Fexport(string ref_filename, string out_filename, ref double[] Dx_vect, ref double[] Dy_vect, ref double report_phi)
        {
            ...
            bool rotate2normal = (settings.export_normalali || (settings.isArbAngle && !out_filename.Contains("_jali."))) && (report_phi!=0);
            ...
            Mat slice;
            Mat warpMat = new Mat(2, 3, MatType.CV_32FC1,0);
            ...
            for (uint i = 0; i < readheader.header.nz; i++)
            {
                ...
                srcTri[0] = new Point2f((float)0.25 * ny, (float)0.25 * nx);
                srcTri[1] = new Point2f((float)0.25 * ny, (float)0.75 * nx);
                srcTri[2] = new Point2f((float)0.75 * ny, (float)0.75 * nx);
                dstTri[0] = new Point2f(y0, x0);
                dstTri[1] = new Point2f(y1, x1);
                dstTri[2] = new Point2f(y2, x2);
                if (rotate2normal)
                {
                    dstTri[0] = new Point2f(0.5f * ny + MathF.Cos(-phi) * (y0 - 0.5f*ny) - MathF.Sin(-phi) * (x0 - 0.5f*nx), 0.5f*nx + MathF.Cos(-phi) * (x0 - 0.5f*nx) + MathF.Sin(-phi) * (y0 - 0.5f*ny));
                    dstTri[1] = new Point2f(0.5f * ny + MathF.Cos(-phi) * (y1 - 0.5f*ny) - MathF.Sin(-phi) * (x1 - 0.5f*nx), 0.5f*nx + MathF.Cos(-phi) * (x1 - 0.5f*nx) + MathF.Sin(-phi) * (y1 - 0.5f*ny));
                    dstTri[2] = new Point2f(0.5f * ny + MathF.Cos(-phi) * (y2 - 0.5f*ny) - MathF.Sin(-phi) * (x2 - 0.5f*nx), 0.5f*nx + MathF.Cos(-phi) * (x2 - 0.5f*nx) + MathF.Sin(-phi) * (y2 - 0.5f*ny));
                }
                Mat warp_mat = Cv2.GetAffineTransform(srcTri, dstTri);
                Scalar border_shade=Cv2.Mean(slice);
                Cv2.WarpAffine(slice, slice, warp_mat, slice.Size(), InterpolationFlags.Linear, BorderTypes.Constant,border_shade);
                WriteImage(i, slice);
            }
            infile.Close();
            outfile.Close();
        ...
        private void WriteImage(uint index, Mat slice)
        {
            int nx = readheader.header.nx;
            int ny = readheader.header.ny;
            Mat slice_rep = new Mat();// ny,nx,MatType.CV_32FC1);

            switch (readheader.header.mode)
            {
                case MRC_MODE_BYTE:
                    {
                        slice.ConvertTo(slice_rep, MatType.CV_8U);
                        ...
                        slice_rep.GetRectangularArray<byte>(out byte[,] arr_slice); //arr_slice = slice.GetData();  //from mat to array
                        byte temp;
                        for (int y = 0; y < ny; y++)
                        {
                            for (int x = 0; x < nx ; x++)
                ...
                case MRC_MODE_SHORT:
                    {
                        slice.ConvertTo(slice_rep,MatType.CV_16S);
                        int bufsize = readheader.header.nx * readheader.header.ny * sizeof(short); //two bytes in int16
                        byte[] tmpbuf = new byte[bufsize];
                        //Array arr_slice;// = new short[ny,nx]; //Nrows, Ncols = y,x    short[ny,nx]
                        slice_rep.GetRectangularArray<short>(out short[,] arr_slice);//  GetData();  //from mat to array
                ...
                case MRC_MODE_USHORT:
                    {
                        slice.ConvertTo(slice_rep, MatType.CV_16U);
                        ...
                        slice_rep.GetRectangularArray<ushort>(out ushort[,] arr_slice); //arr_slice = slice.GetData();  //from mat to array
                        byte[] temp = new byte[2];
                        for (int y = 0; y < ny; y++)
                        {
                            for (int x = 0; x < nx; x++)
                ...
                case MRC_MODE_FLOAT:
                    {
                        ...
                        slice.GetRectangularArray<float>(out float[,] arr_slice); //arr_slice = slice.GetData();  //from mat to array
        ...
        private Mat ReadImage(uint index)
        {
            ...
            //IntPtr imptr = new IntPtr();
            Mat matimptr = new Mat(nx, ny, MatType.CV_32FC1);
            ...
            case MRC_MODE_BYTE:
                {
                    ...
                    infile.Read(tmpbuf, 0, nx * sizeof(byte)); //buffer, offset in buffer, size
                        for (int x = 0; x < nx; x++) {
                            matimptr.Set<float>(x,y, tmpbuf[x] / 255.0f);//CvInvoke.cvSetReal2D(imptr, x, y,tmpbuf[x] / 255.0f); //convert from byte to the opencv data already defined float
                        }
                    }
                break;
                 }
            case MRC_MODE_SHORT: //16 bits signed
                 {
                        ...
                        for (int y = 0; y < ny; y++)
                        {
                            ...
                            {
                                matimptr.Set<float>(x / sizeof(short), y, BitConverter.ToInt16(tmpbuf, x));//CvInvoke.cvSetReal2D(imptr, x/sizeof(short), y, (BitConverter.ToInt16(tmpbuf, x))); //two bytes to int16
                            }
                        }
                        break;
                 }
            case MRC_MODE_USHORT: //16 bits unsigned
                 {
                        ...
                        for (int y = 0; y < ny; y++)
                        {
                            ...
                            for (int x = 0; x < nx * sizeof(short); x += 2)
                            {
                                matimptr.Set<float>(x / sizeof(short), y, BitConverter.ToUInt16(tmpbuf, x));//CvInvoke.cvSetReal2D(imptr, x/ sizeof(short), y, (BitConverter.ToUInt16(tmpbuf, x))); //two bytes to uint16
                            }
                        }
                        break;
                 }
            case MRC_MODE_FLOAT:
                 {
                        ...
                        for (int y = 0; y < ny; y++)
                        {
                            ...
                            {
                                matimptr.Set<float>(x / sizeof(float), y, BitConverter.ToSingle(tmpbuf, x));//CvInvoke.cvSetReal2D(imptr, x/ sizeof(float), y, (BitConverter.ToSingle(tmpbuf, x))); //4bytes bytes to float
                            }
                        }
            ...
            return matimptr; //CvInvoke.CvArrToMat(imptr);
    }
